...
def calculate_rows(
    ...
    for month in range(0, months):
        ...
        interest_ratio = (
            "NaN" if current_net_worth == 0 else total_interest / current_net_worth
        )

        yield {
            "month": month + 1,
            "year": int(month / 12) + 1,
            "invested": total_payments,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--initial_amount", type=float, default=0, help="Initial net worth."
    )
    parser.add_argument(
        "--monthly_savings",
        type=float,
    ...
    parser.add_argument(
        "--output",
        type=str,
        help="File, to which the output is written."
